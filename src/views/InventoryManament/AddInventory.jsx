...
export default function AddInventry({}) {
  ...
  const inventoryData = useSelector(({ selectedData }) => selectedData.inventoryData);
  // const { files } = watch();
  ...
  useEffect(() => {
    ...
    if (inventoryData) {
      Object.keys(inventoryData).forEach((key) => setValue(key, inventoryData[key]));
    }
    return () => {
      dispatch({
  ...
  const onSubmit = (data) => {
    ...
    Object.keys(data).forEach((key) => formData.append(key, key === "image" ? data[key][0] : data[key]));
    if (!isUpdate) {
      addProduct(formData)
      .then(({ data }) => {
        if (data.status) {
      ...
      <CardBody>
        ...
        <FormProvider {...methods}>
          <form onSubmit={handleSubmit(onSubmit)}>
            <GridContainer>
              <GridItem item xs={6}>
                <Custome_Input style={{ width: "100%" }} name="productName" label="Product Name" isrequired />
                <Custome_Input style={{ width: "100%" }} name="productBrand" label="Product Brand" isrequired />
                <Custome_Input
              name="petType"
              label="Pet Type"
             ...
             isrequired/>
                ...
                <Custome_Input name="productDescription" label="Product description" isrequired />
                <Custome_Input name="shelfLife" label="Shelf Life in number of days" type="number" isrequired />
                <Custome_Input
                  name="unit"
                  label="Unit"
                  isrequired
                ...
                />
                <Custome_Input name="availableQuantity" label="Quanity available" type="number" isrequired />

                <Custome_Input name="sellingPrice" type="number" label="Selling Price for Retail Customer" isrequired />
                {/* <Custome_Input
                ...
                <Custome_Input name="mrp" type="number" label="MRP" isrequired />
                <Custome_Input
                  name="gst"
                  label="GST"
                  isrequired
                ...
                <Custome_Input name="subUnit" type="number" label="Subunits Per Unit" />
              </GridItem>
              <GridItem item xs={6}>
                <Custome_Input
                  name="image"
                ...
                label="Sub Category" 
                type="select" 
                option={options !== undefined ? options.map(({ title }) => ({ label: title, value: title })) : [{label:'Select', value:'Select'}]}/>
                <Custome_Input
                  name="vegNonVeg"
                  label="Veg / Non-Veg"
                  isrequired
                ...
                />
                {/* <Custome_Input name="priceOfPurchase" type="number" label="Purchase Price" isrequired /> */}
                {/* <Custome_Input name="HSNNumber" type="number" label="HSN Number" /> */}
                <Custome_Input
                  name="country"
                  label="Country Of Orgin"
                  isrequired
                  type="select"
                  option={countryData.map(({ name }) => ({ label: name, value: name }))}
                />
                <Custome_Input name="perishable" type="select" label="Perishable / Non-Perishable"  option={[{label:"Perishable", value:"Perishable"}, {label:"Non-Perishable", value:"Non-Perishable"},{label:"Not Applicable", value:"N/A"}]} isrequired/>
